...
export interface PostCreateReqeustDto {
...
interface GetPostCreateResponse {
...
const CommunityPage = () => {
  ...
  // const [posts, setPosts] = useState<PostList>(postData.data.posts);
      ...
      if (response.data && response.data.data) {
        ...
        if (file) {
          ...
          if (presignedUrl) {
            ...
            alert("updated:" + updatedPost.data.data.s3ImageUrl);
          }
        }
      }
  ...
  //     setActiveFeed("ALL");
  //   } catch (error) {
  //     console.error("전체 피드 가져오기 실패:", error);
  //   }
  // };
  ...
  const fetchMyPosts = async () => {
    try {
      ...
      setSearchResults([]); // 검색 결과 초기화
      setActiveFeed("USER");
      // alert(JSON.stringify(response.data.data.posts));
            ...
            {recommendedChannels.map((channel, index) => (
              ...
              >
                <div>
                  ...
                  {/* <span className="px-3 py-1 bg-blue-100 text-blue-600 text-sm font-medium rounded-full mr-2">
                    직무
                  </span>
                ...
                >
                  ...
                  <div className="relative w-full h-96">
                    <Image
                      src={
                        post.s3ImageUrl
                        // ? `https://ktb-8-dev-bucket.s3.ap-northeast-2.amazonaws.com/${post.s3ImageUrl}`
                        // : "/surprised_logo_gray.png"
                      }
